...
from src.models.decoder import decoder_final
from src.models.decoder_loss import Loss
import os
...
def parse_args():
    ...
    parser.add_argument("--checkpoint_dir", type=str, default='./data/checkpoints/LRW_18_mstcn_MVM')
...
def train(v_front, a_front, mem, tcn, train_data, epochs, optimizer, scheduler, args):
    ...
    for epoch in range(args.start_epoch, epochs):
        ...
        for i, batch in enumerate(dataloader):
            ...
            te_pred = tcn((te_fusion, target))
            tr_pred = tcn((tr_fusion, target))

            dec_los = Loss(te_pred, target)
            dec_los += Loss(tr_pred, target)
            ...
            tot_loss = recon_loss + contrastive_loss + dec_los
...
def validate(v_front, a_front, mem, tcn, fast_validate=False, epoch=0, writer=None):
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            te_m_pred, _, _ = tcn.inference(te_fusion)
